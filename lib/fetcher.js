...
function writeFile(type , object) {
  return new Promise(function(resolve, reject){
    const file = `./out/${type}-${new Date().toISOString().replace(/T/, '_').replace(/\:/g, '').replace(/\..+/, '')}.json`;
  ...
  return new Promise(function(resolve,reject) {
    ...
    fs.readFile(`./out/${recent}`,'utf8', function(err, data) {
      ...
      async.eachLimit(data, 15, (myUrl, cb) => {
        ...
        const filepath = './out/attachments/' + parsedUrl.hostname + parsedUrl.pathname;

        let justTheDir = filepath.split('/');
        justTheDir.pop();
        justTheDir = justTheDir.join('/');
        mkDirByPathSync(justTheDir);
        if (fs.existsSync(filepath)) {
          counter ++;
          console.log(`\tFile ${filepath} already exists!`);
          console.log(`\t${data.length-counter} file(s) remaining\n`);
          return cb();
        }
        const file = fs.createWriteStream(filepath);
        console.log('\tBegin downloading ', myUrl);
        console.log(`\tBegin downloading ${counter}/${data.length} files`);
        protocol.get(myUrl, function(response) {
          response.pipe(file);
          file.on('finish', function() {
            console.log('\tFinished downloading ', myUrl);
            console.log(`\tFinished downloading ${counter++}/${data.length} files\n`);
            file.close(cb);
          });
        });
  ...
  return co(function*(){
    ...
    return fetchContacts(args,cb)
      ...
      let test = yield fetchAllPages(_.merge({}, args, {
        type,
        count: 0,
        page: args.page && !isNaN(args.page)?args.page:201,
        path: `/api/v1/${type}`,
      }));
      ...
      test = yield fetchAllPages(_.merge({}, args, {
        type,
        count: 0,
        page: args.page && !isNaN(args.page)?args.page:145,
        path: `/api/v1/${type}`,
      }));
      ...
      test = yield fetchAllPages(_.merge({}, args, {
        type,
        count: 0,
        page: args.page && !isNaN(args.page)?args.page:801,
        path: `/api/v1/${type}`,
      }));
